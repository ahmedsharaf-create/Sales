...
const getFirebaseConfig = () => {
  ...
  // IMPORTANT: Ensure these values match the NEW project you are testing
  return {
    apiKey: "", // Platform will inject this at runtime, or paste your key here for local testing
...
export default function App() {
  ...
  useEffect(() => {
    if (!auth) {
      setError("Authentication service unavailable. Please check your Firebase API Key settings.");
      setLoading(false);
      return;
    }
    ...
    const initAuth = async () => {
      try {
        ...
        setError("Identity server connection failed. Ensure Auth Providers are enabled in the project.");
      }
    };
    initAuth();
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        setError("Database Permission Error (Firestore Rules). Please ensure rules allow reading the collection.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6 text-center">
      <div className="max-w-md space-y-4">
        ...
        <h2 className="text-xl font-black uppercase italic tracking-tighter text-slate-800">System Alert</h2>
        ...
        <div className="pt-4 flex flex-col gap-2">
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Ensure Auth and Firestore Rules are enabled in your project</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
        <div className="flex items-center gap-3">
          ...
          <div><h2 className="text-2xl font-black text-slate-800 tracking-tight italic">{isAdmin ? "Performance Center" : "Your Performance"}</h2></div>
        </div>
        {isAdmin && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 no-print">
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-6 border-b border-slate-50 flex justify-between items-center"><h3 className="font-black text-slate-800 text-xs uppercase tracking-widest flex items-center gap-2"><Clock size={16} className="text-red-500" /> Recent Activities</h3></div>
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400"><tr className="tracking-widest"><th className="px-6 py-4">Area Manager</th><th className="px-6 py-4">Last Sales Submission</th></tr></thead>
            <tbody className="divide-y divide-slate-50">
              {managerSummary.length === 0 ? (
                <tr><td colSpan="2" className="px-6 py-8 text-center text-slate-300 font-bold italic">No data for this region.</td></tr>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-8 border-b border-slate-50 space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h3 className="font-black text-slate-800 text-lg tracking-tight">Regional Performance Summary</h3>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredManagerSummary.length === 0 ? (
                <tr><td colSpan="9" className="px-8 py-10 text-center text-slate-300 italic font-bold">No matching records found.</td></tr>
  ...
  return (
    <div className="max-w-2xl mx-auto py-10">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-12 rounded-[3.5rem] shadow-2xl space-y-8 border border-slate-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <input required type="number" placeholder="GA Achieved" className="w-full bg-red-50 p-6 rounded-[2rem] text-3xl font-black text-red-600 outline-none border border-red-100" value={formData.gaAch} onChange={e => setFormData({...formData, gaAch: e.target.value})} />
          <input required type="number" placeholder="OC Achieved" className="w-full bg-blue-50 p-6 rounded-[2rem] text-3xl font-black text-blue-600 outline-none border border-blue-100" value={formData.ocAch} onChange={e => setFormData({...formData, ocAch: e.target.value})} />
        </div>
        <textarea placeholder="Shift notes / feedback..." className="w-full bg-slate-50 p-4 rounded-2xl min-h-[100px] outline-none" value={formData.note} onChange={e => setFormData({...formData, note: e.target.value})} />
        {success && <div className="p-4 bg-emerald-50 text-emerald-600 rounded-2xl text-[10px] font-black text-center uppercase">Saved Successfully</div>}
        <button type="submit" disabled={submitting} className="w-full bg-slate-900 text-white py-6 rounded-[2rem] font-black text-xl shadow-xl hover:bg-black transition-all">Submit Sales</button>
      </form>
    </div>
  );
}
        ...
        <table className="w-full text-left">
          <thead className="bg-[#0F172A] text-slate-400 text-[10px] font-black uppercase tracking-widest">
            <tr className="tracking-widest">
              ...
              <th className="px-8 py-6 text-center">GA Achieved</th>
              <th className="px-8 py-6 text-center">% Completion</th>
              <th className="px-8 py-6 text-center">OC Achieved</th>
          ...
          <tbody className="divide-y divide-slate-50 font-bold tabular-nums">
            {filtered.length === 0 ? (
              <tr><td colSpan={role === 'admin' ? 9 : 8} className="px-8 py-10 text-center text-slate-300 italic">No records found.</td></tr>
        ...
        {users.map(u => (
          <div key={u.uid} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col gap-4 group relative">
            ...
            {editingId === u.uid ? (
              ...
              <button onClick={() => { setEditingId(u.uid); setEditForm({ username: u.username, role: u.role, assignedManager: u.assignedManager || '' }); }} className="w-full bg-slate-50 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all">Edit Permissions</button> 
            )}
          </div>
        ))}
      </div>
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col gap-4">
          ...
          <div className="flex gap-2"><input value={newM} onChange={e => setNewM(e.target.value)} className="flex-1 bg-slate-50 p-4 rounded-xl font-bold outline-none" placeholder="Name" /><button onClick={() => { update([...areaManagers, newM], null); setNewM(''); }} className="bg-red-600 text-white px-6 rounded-xl font-black shadow-lg">Add</button></div>
        </div>
        <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col gap-4">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0F172A] p-4">
      <div className="w-full max-w-md bg-white rounded-[3rem] p-10 shadow-2xl">
        <div className="text-center mb-10"><ShieldCheck className="text-red-600 mx-auto mb-6" size={60} /><h1 className="text-3xl font-black text-slate-800 italic tracking-tighter">Sales Portal</h1><p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">{authMode === 'forgot' ? 'Account Recovery' : 'Secured Access'}</p></div>
...
function Navigation({ view, setView, role, onLogout }) {
  const links = [ { id: 'dashboard', label: 'Dashboard', icon: BarChart3, roles: ['admin', 'user'] }, { id: 'collection', label: 'Entry', icon: PlusCircle, roles: ['admin', 'user'] }, { id: 'reports', label: 'Audit', icon: ClipboardList, roles: ['admin', 'user'] }, { id: 'targets', label: 'Targets', icon: Target, roles: ['admin'] }, { id: 'userSearch', label: 'Team', icon: UsersIcon, roles: ['admin'] }, { id: 'admin', label: 'Config', icon: Settings, roles: ['admin'] } ];
  return (
...
function LoadingScreen() { return ( <div className="flex h-screen items-center justify-center bg-slate-50"><div className="text-center"><Loader2 className="w-12 h-12 animate-spin text-red-600 mx-auto mb-4" /><p className="text-slate-500 font-black text-xs uppercase tracking-widest">Syncing Cloud Assets...</p></div></div> ); }
